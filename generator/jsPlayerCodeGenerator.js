...
Main.__name__ = true;
Main.main = function() {
	haxe_Log.trace(Main.main,{ fileName : "Main.hx", lineNumber : 29, className : "Main", methodName : "main", customParams : [Main.main]});
	Main.storage = js_Browser.getLocalStorage();
	Main.traceOutput = window.document.getElementById("haxe:trace");
	...
	Main.videoLength = js_Boot.__cast(window.document.getElementById("videoLength") , HTMLInputElement);
	...
	Main.isTestControll = js_Boot.__cast(window.document.getElementById("isTest") , HTMLInputElement);
...
Main.validateTimeInput = function(input) {
...
Main.onClick = function() {
	...
	if(Main.validateTimeInput(timeString)) {
		Main.output.value = "wrong time format";
		return;
	}
	lengthString = Main.videoLength.value;
	if(Main.validateTimeInput(lengthString)) {
		Main.output.value = "wrong videl length format";
		return;
	} else {
		var tHours = 0;
		var tMinutes = 0;
		var tSeconds = 0;
		var lengthParts = lengthString.split(":");
		if(lengthParts.length == 1) lengthParts.unshift("00");
		if(lengthParts.length == 2) lengthParts.unshift("00");
	...
	var startTime = HxOverrides.strDate(timeString).getTime();
